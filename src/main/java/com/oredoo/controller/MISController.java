...
public class MISController {
	...
	public String getmisData(Model model, HttpSession session, @RequestParam("partner") String partner,
		...
		if (type == 2) {
			...
			LOGGER.info("INSIDE TYPE 2"+objGetMISData.toString());
			...
			if (source == 20) {
				...
				LOGGER.info("INSIDE non-web Games } "+objGetMISData.toString());
			...
			if (source == 20) {
				...
				LOGGER.info("Source20-INSIDE web Games } "+objGetMISData.toString());
				...
				objGetMISData2 = objportalService.getMisData1(partner, month, month2, type, source);
				objGetMISData.addAll(objGetMISData2);
				LOGGER.info("Source20-INSIDE APP Games } "+objGetMISData.toString());
				...
				objGetMISData.addAll(objGetMISData2);
				objGetMISData.addAll(objGetMISDataweb);
				LOGGER.info("Source20-INSIDE ALL CAse } "+objGetMISData.toString());
		...
		LOGGER.info("Source20-INSIDE ALL CAse } "+listParter.toString());
		...
		while (it2.hasNext()) {
			...
			LOGGER.info("Final Object " + misObj.toString());
	...
	public String contentDetails(Model model, HttpSession session, @RequestParam("id") Integer id,
		...
		if (type == 2) {
			...
			if (source.trim().equalsIgnoreCase("App")) {
				objGetMISData = objportalService.getMisDataContent(id, datefrom, dateto, type, 24);
			} else {
				objGetMISData = objportalService.getMisDataContent(id, datefrom, dateto, type, 20);
			}
			Iterator<Object> it2 = objGetMISData.iterator();
			System.out.println("MSize:" + it2.hasNext());
			...
			while (it2.hasNext()) {
				...
				LOGGER.info("Final Content Object" + misObj.toString());
		...
		System.out.print(misuser.size()+"SIZE MISUSER");
		model.addAttribute("totalUsage", total_usage);
